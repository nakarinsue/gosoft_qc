...
from ..database.models.postgres_models import MPromotionHeader,MFileMaster,MaUser,MInfoImportFile
from ..schemas.inventory import AssignUpdate, ImportInformation, InfoImportCreate, requserassign, EntityStatusUpdate, UpdateAssignRequest
...
from ..common.workload_service import WorkloadDistributor, get_workload_data, format_file_master_data
...
def update_info_status(data: EntityStatusUpdate, db: Session = Depends(get_db)):
    # ค้นหาข้อมูลตาม id
    info_record = db.query(MInfoImportFile).filter(MInfoImportFile.id == data.id).first()
    ...
    try:
        ...
        info_record = (
            db.query(
                ...
                MaUser.username,
                MInfoImportFile.description.label("Remark")
            )
            .join(MInfoImportFile, MFileMaster.v_id == MInfoImportFile.id)
            .join(MaUser, MFileMaster.user_create == MaUser.user_id)
            .filter(MInfoImportFile.v_id != Version if Version == 0 else MInfoImportFile.v_id == Version)
...
def assign_users_calc(req: requserassign, db: Session = Depends(get_db)):
    try:
        ...
        flat_list, diff, summary_dict = distributor.distribute_strict_balance(user_ids=req) # type: ignore
