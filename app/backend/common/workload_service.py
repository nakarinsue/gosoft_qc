...
from ..database.models.postgres_models import MPromotionHeader,MFileMaster,MPromotionBucketEntity
from sqlalchemy import func
from typing import Dict, List, Any,Optional
from ..schemas.inventory import requserassign
...
def get_before_transation(db: Session,Version:int =0):
    info_record = (
        db.query(
...
class WorkloadDistributor:
    ...
    def distribute_strict_balance(self, user_ids:Optional[autoassignrequre]|None =None):
        """
        user_ids: List ของ ID ผู้ใช้งาน เช่น [1, 2, 3] หรือ ['1', '2', '3']
        """
        if not user_ids :
            return [], 0, {}
        elif not user_ids.user_id:
            return [], 0, {}

        sorted_df = self.raw_data.sort_values(by='VALUE', ascending=False).reset_index(drop=True)
        
        bins = [[] for _ in range(len(user_ids.user_id))]
        bin_sums = [0] * len(user_ids.user_id)
        
        for _, row in sorted_df.iterrows():
            min_idx = bin_sums.index(min(bin_sums))
            item = row.to_dict()
            item['ASSIGNED_TO'] = str(user_ids.user_id[min_idx]) 
        ...
        summary = {str(user_ids.user_id[i]): bin_sums[i] for i in range(len(user_ids.user_id))}
