...
from app.backend.models.postgres._base_on import MFileMaster, MPromotionHeader, MPromotionBucketEntity
from app.config import settings as config
from app.backend.schemas.all_schemas import entity_id, Product_code
...
class FullImportService:
    ...
    def run_process(self, file_content: bytes, filename: str, version: str, system: str, user_id: int, remark: str = ""):
        """Main Flow การทำงาน"""
        try:
            ...
            for sheet_name, df in excel_data.items():
                self._process_sheet(df, sheet_name, filename, version, s3_path, user_id, remark, visible_sheet_names)
    ...
    def _process_sheet(self, df: pd.DataFrame, sheet_name: str, filename: str, version: str, s3_path: str, user_id: int, remark: str, sheet_hide: List[str|None] = [None]):
        ...
        remark = self._check_error_show_description(df=df, sheet_name=sheet_name, sheet_hide=sheet_hide)
        ...
        for ind, p_code in enumerate(unique_promos):
            ...
            try:
                ...
                print(remark)
        try:
            barcode_map = self._product(product_code) 
            if barcode_map:
                items_to_update = self.db.query(MPromotionBucketEntity)\
                                         .filter(MPromotionBucketEntity.entity_type.in_(barcode_map.keys()))\
                                         .filter(MPromotionBucketEntity.barcode == '')\
                                         .all()
                if items_to_update:
                    for i in items_to_update:
                        # จับคู่ O(1) หากเจอ Barcode ให้ทำการอัปเดต
                        new_barcode = barcode_map.get(i.entity_code)
                        if new_barcode:
                            i.barcode = new_barcode
                            
                    self.db.commit()

        except Exception as e:
            self.db.rollback()
            print(f"Error in Finalize Status (Barcode Mapping): {e}")
    ...
    def _create_promotion_item(self, row: dict, header_id: int, user_id: int):
        ...
        return MPromotionBucketEntity(
            ...
            condition=str(row.get('condition', ''))[:300],
