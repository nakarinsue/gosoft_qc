...
from app.config import settings as config  # สมมติว่ามี config เก็บค่า Minio
from app.backend.models.postgres._base_on  import (
    MVersionControl, MFileMaster, MPromotionHeader, 
    MPromotionBucketEntity, MaUser
)
from app.backend.schemas.all_schemas import entity_id,Product_code,ImportInformation,Optional
...
class PromotionImportService:
    ...
    def read_excel_from_minio(self, path_file: str):
        ...
        """
        storage_options = {
            "key": config.MINIO_ACCESS_KEY,          # Access Key
            "secret": config.MINIO_SECRET_KEY,   # Secret Key
            "client_kwargs": {
