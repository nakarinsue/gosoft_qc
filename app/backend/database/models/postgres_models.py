...
from sqlalchemy import (
    Integer, String, Boolean, DateTime, Date, Text, func,
    ForeignKey, UniqueConstraint, Index, MetaData, PrimaryKeyConstraint
)
from sqlalchemy.orm import Mapped, mapped_column, relationship

# สมมติฐานว่าดึง Base มาจาก backend.database หรือ declarative_base
...
class MVersionControl(Base):
    ...
    __table_args__ = (
        # UniqueConstraint("sr_no", name="ix_sr_no_mi_version"),
        {"schema": POSTGRE_SCHEMA}
    )
    ...
    user_create: Mapped[int] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.u_user.user_id"), nullable=False)
    date_update: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    user_update: Mapped[int] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.u_user.user_id"), nullable=False)

    # --- Relationships ---
    # 1 Version มีข้อมูล Import File ได้หลายไฟล์
...
class PPayment(Base):
    ...
    user_update: Mapped[Optional[int]] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.u_user.user_id"))
...
class MInfoImportFile(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    v_id: Mapped[int] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.m_version_control.id", ondelete="CASCADE"), nullable=False)
    status: Mapped[int] = mapped_column(Integer, nullable=False)
    description: Mapped[Optional[str]] = mapped_column(String(255))
...
class MFileMaster(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    v_id: Mapped[int] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.m_info_import_file.id", ondelete="CASCADE"), nullable=False)
...
class MPromotionHeader(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    file_id: Mapped[int] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.m_file_master.id", ondelete="CASCADE"), nullable=False)
    ...
    user_assign: Mapped[Optional[int]] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.u_user.user_id"))
...
class TDefect(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    pro_id: Mapped[int] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.m_promotion_header.id", ondelete="CASCADE"), nullable=False)
    ...
    user_create: Mapped[int] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.u_user.user_id"), nullable=False)
    date_update: Mapped[datetime] = mapped_column( DateTime, server_default=func.now(),  onupdate=func.now(), nullable=False)
    user_update: Mapped[int] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.u_user.user_id"), nullable=False)

    # --- Relationships ---
    itemdefects: Mapped[List["TDefectitem"]] = relationship(
        "TDefectitem", 
        back_populates="defect", 
        cascade="all, delete-orphan",
        foreign_keys="[TDefectitem.df_id]"
    )    
...
class TDefectitem(Base):
    __tablename__ = "t_Defect_item"
    __table_args__ = {"schema": POSTGRE_SCHEMA}
    
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    df_id: Mapped[int] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.t_Defect.id", ondelete="CASCADE"), nullable=False)  
    name: Mapped[str] = mapped_column(Text, nullable=False)
    type_item: Mapped[int] = mapped_column(Integer, nullable=False)
    description: Mapped[Optional[str]] = mapped_column(String(255))
    # --- Relationships ---
    defect: Mapped["TDefect"] = relationship("TDefect", back_populates="itemdefects")
...
class TTransaction(Base):
    ...
    # --- Relationships ---
    promotion: Mapped["MPromotionHeader"] = relationship(f"MPromotionHeader", back_populates="transactions")
...
class MPromotionBucketEntity(Base):
    ...
    pro_id: Mapped[int] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.m_promotion_header.id", ondelete="CASCADE"), primary_key=True)
    ...
    # Product Columns,product_name,barcode,product_price,date_update,product_package
    ...
    t_id: Mapped[Optional[int]] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.t_transaction.id"))
    d_id: Mapped[Optional[int]] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.t_Defect.id"))
...
class TTej(Base):
    ...
    T_id: Mapped[int] = mapped_column(ForeignKey(f"{POSTGRE_SCHEMA}.t_transaction.id"), nullable=False)
