...
export default function MainLayout({ currentView, onViewChange, onLogout, user, children, onError }: any) {
  ...
  const activeMenu: any = allItems.find(item => item && (item as any).id === currentView) || allItems[0];
  ...
  const handleUpdateProfile = async (e: any) => {
    e.preventDefault();
    try {
      // Mocking fetch for layout
      alert("อัปเดตข้อมูลสำเร็จ! ระบบจะทำการ Refresh ข้อมูล (Mock)");
  ...
  const fetchVersions = useCallback(async () => {
    try {
      // Mock fetch versions
      setVersions([{ id: 1, title: 'v1.0' }, { id: 2, title: 'v2.0' }]);
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-white overflow-hidden">
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div className={`p-3 rounded-2xl text-white ${activeMenu?.bg} shadow-xl ${activeMenu?.shadow} transition-all`}>
              {activeMenu?.Icon && <activeMenu.Icon size={24} />}
            </div>
            <div>
      ...
      <div className="flex flex-1 relative overflow-hidden">
        ...
        >
          <nav className="flex-1 px-4 space-y-8 overflow-y-auto custom-scrollbar pb-20">
              ...
              <div key={gIdx} className="space-y-3">
                ...
                {group.items.map((item: any) => {
                  ...
                  return (
                    ...
                    >
                      {/* 📍 ขยายขนาด Icon Sidebar เป็น 28 */}
                      <item.Icon size={28} className={isActive ? 'text-white scale-110' : `${item.color} group-hover:scale-110 transition-transform duration-300`} />
            ...
            <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className={`w-full flex items-center gap-4 p-3 rounded-3xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 hover:border-blue-500 transition-all duration-300 ${!isSidebarOpen && 'justify-center'}`}>
              ...
              {isSidebarOpen && (
                <div className="flex-1 text-left overflow-hidden">
                  <p className="text-sm font-black truncate text-slate-800 dark:text-white">{user?.name || 'User Name'}</p>
        ...
        {/* Workspace */}
        <main className="flex-1 ml-24 h-full p-8 bg-slate-50/50 dark:bg-slate-950/50 overflow-auto">
          <div className="w-full h-full max-w-7xl mx-auto flex flex-col">
              {React.Children.map(children, (child: any) => 
                React.isValidElement(child) ? React.cloneElement(child, { selectedversion: selectedVersion } as any) : child
              )}
          </div>
        </main>
      </div>
