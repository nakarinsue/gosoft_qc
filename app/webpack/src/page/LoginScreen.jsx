...
import { FileText, User, Lock, Loader2, Check, ArrowRight } from 'lucide-react';
...
export default function LoginScreen({ onLogin, onError, onViewChange }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 📍 เรียก API Login ผ่าน apiService กลาง (ไม่ต้องสร้าง URLSearchParams เองแล้ว)
      const data = await apiService.auth.login(username, password);

      // Interceptor ของคุณทำการ return response.data มาให้แล้ว จึงเช็ค property ได้เลย
      if (data && data.access_token) {
        // 📍 บันทึก Token ลง Storage (Enterprise Standard)
        localStorage.setItem('access_token', data.access_token);
        localStorage.setItem('token_type', data.token_type || 'bearer');
        localStorage.setItem('role', data.role || 'user');
        localStorage.setItem('allmember', data.allmember || '');
      ...
      // ข้อความ Error ถูกจัดการมาจาก Interceptor ใน apiService แล้ว
