...
from Storage import Llistout
from typing import List, Any, Optional, Dict



class data():
    def getdata(): # type: ignore
        return "146"
class DATA():
    ...
    def sum(self):
        ...
        return [Temp,step]
    @staticmethod
    def _process_transaction_Tx(data: str) -> Dict[str, Any]:
        """
        [ฟังก์ชันย่อย] จัดการ String ตัดข้อความ แยกด้วย '|' 
        และคำนวณ Sequence ใหม่แบบ 5 หลัก พร้อมจัดเก็บลง Dictionary
        """
        split_data_str = data.split("|")
        commontran_list: List[str] = []
        for item in split_data_str:
            cut_item = item[3:]
            try:
                calc_val = int(cut_item) + len(split_data_str)
            except ValueError:
                continue 
            commontran_list.append(str(calc_val).zfill(5))
        return {
            "tx_id_list": split_data_str,
    ...
    def Editdata(self):
        ...
        for i in enumerate(Llistout()) :
            if i[0] != 0:
                try :data_output_temp.append(DATATOSTA[Llistout()[0]][Llistout()[i[0]]])
                except:data_output_temp.append('')
        SEQ_NO1 = str(str(data_output_temp[5]).replace(data.getdata(),"")) 
        TX_ID2 = data_output_temp[5][:8]
        if len(SEQ_NO1) == 5:
            sumSEQ = CHECK_Length(str(int(SEQ_NO1)+1))
            sumAMT = str(float(data_output_temp[8])+2)
        elif len(SEQ_NO1) != 5:  
        ...
        return [TX_ID2,SEQ_NO1,sumSEQ,dummyDATA,sumAMT,dummyDATA,data_output_temp]
