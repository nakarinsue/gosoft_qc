    ...
    try:
        ...
        for i in range(0, len(clean_items), CHUNK_SIZE):
            ...
            """
                ...
                #     PS.SALE_PRICE,
                #     MAX(B.BARCODE)
                # FROM [POSG2].[dbo].[MS_PRODUCT] P
                # INNER JOIN [POSG2].[dbo].[MA_BARCODE] B ON P.PRODUCT_CODE = B.PRODUCT_CODE
                # INNER JOIN [POSG2].[dbo].[MS_PRICE_SALE] PS ON P.PRODUCT_CODE = PS.PRODUCT_CODE
                # LEFT JOIN [POSG2].[dbo].[MS_PMA] PM ON PM.PMA_CODE = P.PMA_CODE
                # LEFT JOIN [POSG2].[dbo].[MS_CATEGORY] C ON C.PSA_CODE = P.PMA_CODE AND C.CATEGORY_CODE = P.CAT_PMA
                # LEFT JOIN [POSG2].[dbo].[MS_PRODUCT_CONVERT] PC ON PC.PRODUCT_SOURCE = P.PRODUCT_CODE
                # LEFT JOIN [POSG2].[dbo].[MS_PRODUCT_ZERO_PRICE] ZP ON ZP.PRODUCT_CODE = P.PRODUCT_CODE
                # WHERE P.PRODUCT_CODE IN ({placeholders})
				# AND NOT (ZP.PRODUCT_CODE IS NULL AND PS.SALE_PRICE = 0.0)
				# GROUP BY  P.PRODUCT_CODE,P.PMA_CODE,CAT_PMA,SUBCAT_PMA,PRODUCT_NAME,SALE_PRICE
                # ORDER BY  PRODUCT_CODE, PRODUCT_NAME, SALE_PRICE DESC;
            # ส่ง Chunk ย่อยเข้าไป Query
