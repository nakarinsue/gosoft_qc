...
class DATA():
    ...
    def Editdata(self):
        DATATOSTA=xd.parse(self.Editout) # type: ignore
        dummyDATA = None
        v= []
        ...
        SEQ_NO1 = str(str(v[5]).replace(data.getdata(),"")) # type: ignore
        TX_ID2 = v[5][:8]
        if len(SEQ_NO1) == 5:
            sumSEQ = CHECK_Length(str(int(SEQ_NO1)+1))
            sumAMT = str(float(v[8])+2)
        elif len(SEQ_NO1) != 5:  
            BILL_AMT0 = self.Editout.split("<BILL_AMT")[-1].split("/BILL_AMT>")[0]    # type: ignore
            ...
            sumSEQ  = SEQ_NO2+'|'+ SEQ_NO3 # type: ignore
